...
export class SlackService {
  static async setUserStatus(userId: string, statusText: string, statusEmoji: string, expiration?: number): Promise<MultiWorkspaceResponse> {
    ...
    const connections = await this.getAllActiveUserConnections(userId)
  ...
  static async clearUserStatus(userId: string): Promise<MultiWorkspaceResponse> {
    const preferences = await this.getUserPreferences(userId)
  ...
  static async enableDnd(userId: string, durationMinutes: number): Promise<MultiWorkspaceResponse> {
    const preferences = await this.getUserPreferences(userId)
  ...
  static async disableDnd(userId: string): Promise<MultiWorkspaceResponse> {
    const preferences = await this.getUserPreferences(userId)
  ...
  static async getDndInfo(userId: string): Promise<any> {
    ...
    for (const connection of connections) {
      ...
      try {
        ...
        workspaceResult.next_dnd_start_ts = data.next_dnd_start_ts || null
        workspaceResult.next_dnd_end_ts = data.next_dnd_end_ts || null
  ...
  static async startFocusSession(userId: string, sessionId?: string, durationMinutes?: number): Promise<MultiWorkspaceResponse> {
    const preferences = await this.getUserPreferences(userId)
  ...
  static async endFocusSession(userId: string, sessionId?: string): Promise<MultiWorkspaceResponse> {
    const preferences = await this.getUserPreferences(userId)
    ...
    const actualSessionId = sessionId || await this.getActiveSessionId(userId)
    ...
    for (const sessionWorkspace of sessionWorkspaces) {
      ...
      try {
        ...
        // Log successful activities
        if (workspaceResult.status_updated) {
          await this.logActivity(userId, 'status_cleared', sessionWorkspace.workspace_id, {
            session_id: actualSessionId
          })
        }

        if (workspaceResult.dnd_enabled) {
          await this.logActivity(userId, 'dnd_disabled', sessionWorkspace.workspace_id, {
            session_id: actualSessionId
          })
        }
  ...
  private static async getActiveSessionId(userId: string): Promise<string | null> {
  ...
  static async getFocusSessionStatus(userId: string): Promise<any> {
    const preferences = await this.getUserPreferences(userId)
  ...
  private static async getAllActiveUserConnections(userId: string): Promise<any[]> {
  ...
  private static async executeWithRetry<T>(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        const mappedError = mapSlackError(slackError)
  ...
  static async cleanupExpiredFocusSession(sessionId: string, userId: string): Promise<void> {
    try {
